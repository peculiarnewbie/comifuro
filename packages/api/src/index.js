...
const legacyTweetSchema = z.array(z.object({
    id: z.string().min(1),
...
function normalizeEventId(value, fallback = "cf21") {
    return value?.trim().toLowerCase() || fallback;
}
function maskToFallbackR2Keys(tweetId, mask, maxBits = 8) {
    const keys = [];
...
async function buildPublicFeed(db, eventId) {
    const publicTweets = await tweetsOperations.listPublicTweets(db, "catalogue", eventId);
    const media = await tweetsOperations.listPublicTweetMedia(db, publicTweets.map((tweet) => tweet.id));
    ...
    return Object.fromEntries(publicTweets.map((tweet) => [
        tweet.id,
        {
...
app.get("/", (c) => c.text("ok"))
    ...
    await c.env.R2.put(parsedKey.data.key, file, {
        httpMetadata: {
    ...
    const imagesByTweet = await tweetsOperations.listTweetImages(db, pageRows.map((row) => ({
        id: row.id,
    ...
    const now = new Date();
    const rows = parsed.data.map((tweet) => ({
        id: tweet.id,
