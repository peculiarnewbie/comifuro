...
export function groupTweetsIntoThreads(tweets: CatalogueTweet[]): CatalogueTweetThread[] {
...
export type AdminActions = {
    saveFandoms: (tweetId: string, value: string, auth: AdminAuth) => Promise<void>;
    saveTags: (tweetId: string, value: string, auth: AdminAuth) => Promise<void>;
    reroot: (groupId: string, tweetId: string, auth: AdminAuth) => Promise<void>;
    uncatalogue: (tweetId: string, auth: AdminAuth) => Promise<void>;
    removeFollowUp: (tweetId: string, auth: AdminAuth) => Promise<void>;
};
...
export async function createTweetStoreSession({
    ...
    const getSnapshot = (): TweetStoreSnapshot => {
        ...
        return {
            ...
            bootstrapComplete: getValue<boolean>(store, VALUE_BOOTSTRAP_COMPLETE, false),
    ...
    const setStatus = (status: SyncStatus) => store.setValue(VALUE_SYNC_STATUS, status);
    const setError = (error: string | null) => store.setValue(VALUE_SYNC_ERROR, error);
    const setBootstrapComplete = (v: boolean) => store.setValue(VALUE_BOOTSTRAP_COMPLETE, v);
    const setLastSyncedAt = (t: number) => store.setValue(VALUE_LAST_SYNCED_AT, t);
    ...
    const syncOnce = async () => {
        ...
        syncPromise = (async () => {
            try {
                const bootstrapped = getValue<boolean>(store, VALUE_BOOTSTRAP_COMPLETE, false);
